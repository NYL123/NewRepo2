...
public class TestTargetTypeCoordinatorServiceImpl {

	@InjectMocks
	private TargetTypeCoordinatorServiceImpl targetTypeCoordinatorServiceImpl;
	...
	public void setUp() throws Exception {
		targetTypeCoordinatorServiceImpl = new TargetTypeCoordinatorServiceImpl();
		...
		PowerMockito.when(mockResResolver.adaptTo(PageManager.class)).thenReturn(pagemanager);
		...
		PowerMockito.when(valueMap.get(HertzConstants.JCR_CQ_TEMPLATE)).thenReturn("location");
		servclass = targetTypeCoordinatorServiceImpl.getClass();
		fields = servclass.getDeclaredFields();
		PrivateAccessor.setField(targetTypeCoordinatorServiceImpl, "hConfigFactory", hConfigFactory);
		PrivateAccessor.setField(targetTypeCoordinatorServiceImpl, "mcFactory", mcFactory);
		PrivateAccessor.setField(targetTypeCoordinatorServiceImpl, "properties", properties);
		String[] basePaths = new String[] { TestConstants.CONTENT_DATA_PATH, TestConstants.CONTENT_MOBILE_DATA_PATH };
		String[] mappingsDataCoord = new String[] { "/rates-config/default-rq-code:rqcodes", "/locations:location",
				"/target-types:targettype" };
		String[] dataApi = new String[] { "rqcodes:/api/rateCodeService/defaultRateCode",
				"location:/api/locationService", "targettype:/api/cacheUploadService/v1/loadcacheinformation" };
		when(hConfigFactory.getPropertyValue(Mockito.eq(HertzConstants.HERTZ_DATA_BASEPATH))).thenReturn(basePaths);
		when(hConfigFactory.getPropertyValue(Mockito.eq(HertzConstants.HERTZ_DATA_COORDINATOR_MAPPING)))
				.thenReturn(mappingsDataCoord);
		when(hConfigFactory.getPropertyValue(Mockito.eq(HertzConstants.HERTZ_DATA_API_MAPPING))).thenReturn(dataApi);

		when(hConfigFactory.getPropertyValue(Mockito.eq(HertzConstants.HERTZ_DATA_TEMPLATE_WHITELIST)))
				.thenReturn(mappingsDataCoord);
	...
	public void testActivate() throws NoSuchMethodException, SecurityException, IllegalAccessException,
		...
		Method activate = targetTypeCoordinatorServiceImpl.getClass().getDeclaredMethod("activate",
				ComponentContext.class);
		activate.setAccessible(true);
		activate.invoke(targetTypeCoordinatorServiceImpl, mockComponentContext);
	}

	@Test
	public void testGetCollatedData() throws JSONException, RepositoryException {
		CoordinatorConsumable consumable = targetTypeCoordinatorServiceImpl
	...
	public void testgetSpecificDataPath() {
		String[] mappingsDataCoord = new String[] { "/rates-config/default-rq-code:rqcodes", "/locations:location",
				"/target-types:targettype" };
		String path = targetTypeCoordinatorServiceImpl.getSpecificDataPath(TestConstants.LOCATION_PATH,
				TestConstants.CONTENT_DATA_PATH, mappingsDataCoord);
		Assert.assertNotNull(path);

		mappingsDataCoord = new String[] { "/rates-config/default-rq-code:", "/locations:" };
		path = targetTypeCoordinatorServiceImpl.getSpecificDataPath(TestConstants.LOCATION_PATH,
				TestConstants.CONTENT_DATA_PATH, mappingsDataCoord);
		Assert.assertNotNull(path);
	}

	@Test
	public void testformRequestParams() throws JSONException, RepositoryException {
		when(mockResResolver.getResource(TestConstants.ERROR_DATA_PATH)).thenReturn(resource);
		...
		when(valueMap1.get("service", TestConstants.TARGET_TYPE_RES
				.substring(TestConstants.TARGET_TYPE_RES.lastIndexOf('/')).replaceAll("/", "")))
		...
		when(resource1.getResourceType()).thenReturn(TestConstants.TARGET_TYPE_RES);
		...
		targetTypeCoordinatorServiceImpl.formRequestParams(TestConstants.ERROR_DATA_PATH, resource);
	}

	@Test
	public void testformRequestParams4() throws JSONException, RepositoryException {
		when(mockResResolver.getResource(TestConstants.ERROR_DATA_PATH)).thenReturn(resource);
		when(resource.getPath()).thenReturn(TestConstants.ERROR_DATA_PATH);
		when(resource.adaptTo(Page.class)).thenReturn(dataPage);
		when(dataPage.getContentResource()).thenReturn(childJcrResource);
	...
	@Test
	public void testformRequestParams1() throws JSONException, RepositoryException {
		when(mockResResolver.getResource(TestConstants.ERROR_DATA_PATH)).thenReturn(resource);
		when(resource.getPath()).thenReturn(TestConstants.ERROR_DATA_PATH);
		when(resource.adaptTo(Page.class)).thenReturn(dataPage);
		when(dataPage.getContentResource()).thenReturn(childJcrResource);
		...
		when(valueMap1.get(HertzConstants.SERVICE, String.class)).thenReturn("inFact");
	...
	@Test
	public void testformRequestParams2() throws JSONException, RepositoryException {
		when(mockResResolver.getResource(TestConstants.ERROR_DATA_PATH)).thenReturn(resource);
		when(resource.getPath()).thenReturn(TestConstants.ERROR_DATA_PATH);
		when(resource.adaptTo(Page.class)).thenReturn(dataPage);
	...
	@Test
	public void testformRequestParams3() throws JSONException, RepositoryException {
		when(mockResResolver.getResource(TestConstants.ERROR_DATA_PATH)).thenReturn(resource);
		when(resource.getPath()).thenReturn(TestConstants.ERROR_DATA_PATH);
