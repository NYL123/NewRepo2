...
public class TestGoldPlusRewardsUse {

	@InjectMocks
	private GoldPlusRewardsUse goldPlusRewardsUse;
	...
	public final void setup() throws Exception{
		...
		goldPlusRewardsUse =PowerMockito.mock(GoldPlusRewardsUse.class);
		Mockito.doCallRealMethod().when(goldPlusRewardsUse).activate();
		Mockito.doCallRealMethod().when(goldPlusRewardsUse).setJsonString(Mockito.anyString());
		Mockito.doCallRealMethod().when(goldPlusRewardsUse).getJsonString();
	}
	
	@Test
	public final void testActivate() throws RepositoryException, JSONException, ServletException, IOException{
		String requestPath = "dummypath";
		String [] seoArray=new String[]{"{'configurableSeoKey':'seoKeywords','configurableSeoValue':'SEO Keywords'}"};
		when(goldPlusRewardsUse.getRequest()).thenReturn(request);
		...
		when(goldPlusRewardsUse.getCurrentPage()).thenReturn(currentPage);
		when(currentPage.getTitle()).thenReturn("Gold Plus Rewards");
		when(goldPlusRewardsUse.getResource()).thenReturn(resource);
		...
		when(propMap.get(JcrConstants.JCR_TITLE,String.class)).thenReturn("Gold Plus Rewards Page");
		when(propMap.get(JcrConstants.JCR_DESCRIPTION,String.class)).thenReturn("description");
		when(goldPlusRewardsUse.getSlingScriptHelper()).thenReturn(scriptHelper);
		...
		goldPlusRewardsUse.activate();
		when(requestPathInfo.getSelectors()).thenReturn(new String[]{"native"});
		goldPlusRewardsUse.activate();
		when(resource.getChild(HertzConstants.CONFIGTEXT_PARSYS)).thenReturn(null);
		goldPlusRewardsUse.activate();
	}
	
	@Test
	public final void testCreateJsonForLinkDropdown() throws JSONException{
		...
		when(linkDropdownResource.getValueMap()).thenReturn(propMap);
		when(propMap.containsKey(HertzConstants.DROPDOWN_LABEL)).thenReturn(true);
		when(propMap.get(HertzConstants.DROPDOWN_LABEL)).thenReturn(dropdownLabel);
		when(propMap.get(HertzConstants.DROPDOWN_VALUE_LIST)).thenReturn(dropDownValueList);
		GoldPlusRewardsUse.createJsonForLinkDropdown(linkDropdownResource, beanMap);
		GoldPlusRewardsUse.createJsonForLinkDropdown(null, beanMap);
		when(propMap.get(HertzConstants.DROPDOWN_VALUE_LIST)).thenReturn(new String[]{});
		GoldPlusRewardsUse.createJsonForLinkDropdown(linkDropdownResource, beanMap);
	}
	...
	public final void testGetterSetters(){
		...
		goldPlusRewardsUse.setJsonString(jsonString);
		Assert.assertTrue(goldPlusRewardsUse.getJsonString().equalsIgnoreCase(jsonString));
	...
	@Test
	public final void testSetLinkDropDownValuesInMap() throws JSONException{
		Map<String,Object> map=new HashMap<>();
		JSONObject object=PowerMockito.mock(JSONObject.class);
		when(object.getString(HertzConstants.DROPDOWN_VALUE)).thenReturn(StringUtils.EMPTY);
		when(object.getString(HertzConstants.ORDER)).thenReturn(StringUtils.EMPTY);
		GoldPlusRewardsUse.setLinkDropDownValuesInMap(map, object);
		when(object.getString(HertzConstants.DROPDOWN_VALUE)).thenReturn("dropdownValue");
		when(object.getString(HertzConstants.ORDER)).thenReturn(StringUtils.EMPTY);
		GoldPlusRewardsUse.setLinkDropDownValuesInMap(map, object);
		when(object.getString(HertzConstants.DROPDOWN_VALUE)).thenReturn(StringUtils.EMPTY);
		when(object.getString(HertzConstants.ORDER)).thenReturn("order");
		GoldPlusRewardsUse.setLinkDropDownValuesInMap(map, object);
		when(object.getString(HertzConstants.DROPDOWN_VALUE)).thenReturn("linkDropDownValue");
		when(object.getString(HertzConstants.ORDER)).thenReturn("order");
		GoldPlusRewardsUse.setLinkDropDownValuesInMap(map, object);
