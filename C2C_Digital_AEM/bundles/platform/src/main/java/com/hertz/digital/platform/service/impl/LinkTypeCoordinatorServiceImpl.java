...
/**
 * This class is used to form the request for the link type notification
 * service.
 * 
 * @author himanshu.i.sharma
 *
 */
@Component(immediate = true, metatype = false, label = "Hertz - Link Type MC Data Coordinator Service", description = "Hertz - Link Type MC Data Coordinator Service")
@Service(value = MCDataCoordinatorService.class)
@Properties(value = @Property(name = "identifier", value = "linktype", propertyPrivate = true))
public class LinkTypeCoordinatorServiceImpl extends AbstractMCDataCoordinator {

	/**
	 * LOGGER instantiation
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(LinkTypeCoordinatorServiceImpl.class);
	 ...
	 */
	public LinkTypeCoordinatorServiceImpl() {
		super();
	}
	...
	private void getJsonBodyParams(JSONObject finalJsonObject, JSONObject jsonObject, Resource contentResource)
		...
		if (HertzUtils.isValidResource(parResource)) {
			...
			JSONArray linkTypeJsonArray = new JSONArray();
				...
				if (subResourceMap.containsKey(HertzConstants.KEY)) {
					...
					object.put(HertzConstants.VALUE,
							StringUtils.defaultIfBlank(
									...
									StringUtils.EMPTY));
					linkTypeJsonArray.put(object);
				}
			}
			linkTypeObject.accumulate(str, linkTypeJsonArray);
			JSONArray jsonArray = new JSONArray();
			jsonArray.put(linkTypeObject);
