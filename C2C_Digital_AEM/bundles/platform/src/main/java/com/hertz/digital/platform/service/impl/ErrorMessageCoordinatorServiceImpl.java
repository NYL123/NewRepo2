...
/**
 * This class is used to form the request for the error message notification
 * service.
 * 
 * @author a.dhingra
 *
...
@Component(immediate = true, metatype = false,
        label = "Hertz - Error Messages MC Data Coordinator Service",
        description = "Hertz - Error Messages MC Data Coordinator Service")
@Service(value = MCDataCoordinatorService.class)
@Properties(value = @Property(name = "identifier", value = "errormessages",
        propertyPrivate = true))
public class ErrorMessageCoordinatorServiceImpl
        extends AbstractMCDataCoordinator {

    /**
     * LOGGER instantiation
     */
    private static final Logger LOGGER = LoggerFactory
            .getLogger(ErrorMessageCoordinatorServiceImpl.class);
     ...
     */
    public ErrorMessageCoordinatorServiceImpl() {
        super();
    }
     ...
     * 
     * @param dataPath
     *            The path being replicated
     * @param resolver
     *            The resolver to resolve the resource
     * 
     */
    @Override
    protected String formRequestParams(String dataPath, Resource dataResource)throws JSONException, RepositoryException {
        ...
        if (HertzUtils.isValidResource(dataResource)) {
            ...
            String extractBrandFrom = dataPath.split("/")[2];
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("correlationId", System.currentTimeMillis());
            jsonObject.put("locale", dataPath.split("/")[4]);
            jsonObject.put("brand", extractBrandFrom.split("-")[0]);
            LOGGER.debug("locale :- " + jsonObject.optString("locale"));
	...
	private void getJsonBodyParams(JSONObject finalJsonObject, JSONObject jsonObject, Resource contentResource)
		...
		if (map.containsKey(HertzConstants.SERVICE)) {
		    jsonObject.put(HertzConstants.SERVICE,
		            map.get(HertzConstants.SERVICE, String.class));
		}
		LOGGER.debug("service :- " + jsonObject.optString(HertzConstants.SERVICE));
		Resource parResource = contentResource.getChild(HertzConstants.PAR);
		if (HertzUtils.isValidResource(parResource)) {
		    setErrorJsonObject(finalJsonObject, jsonObject, parResource);
		}
	}
	 ...
	 * @throws JSONException
	 */
	private void setErrorJsonObject(JSONObject finalJsonObject, JSONObject jsonObject, Resource parResource)
			throws JSONException {
		Iterable<Resource> resourceIterator = parResource.getChildren();
		JSONObject errorObject = new JSONObject();
		JSONArray errorMsgJsonArray = new JSONArray();
		String str = StringUtils.EMPTY;
		for (Resource subResource : resourceIterator) {
		    JSONObject object = new JSONObject();
		    String componentResourceType = subResource.getResourceType();
		    str = componentResourceType.substring(componentResourceType.lastIndexOf('/')).replaceAll("/", "");
		    ValueMap subResourceMap = subResource.getValueMap();
		    if (subResourceMap.containsKey(HertzConstants.ERROR_CODE)&& 
		    		subResourceMap.containsKey(HertzConstants.ERROR_VALUE)) {
		        object.put(HertzConstants.KEY,StringUtils.trim(subResourceMap
		        		.get(HertzConstants.ERROR_CODE,String.class)));
		        object.put(HertzConstants.VALUE,StringUtils.trim(subResourceMap
		        		.get(HertzConstants.ERROR_VALUE,String.class)));
		        errorMsgJsonArray.put(object);
		    }
		    else if (subResourceMap.containsKey(HertzConstants.ERROR_CODE) && 
            		!subResourceMap.containsKey(HertzConstants.ERROR_VALUE)){
                object.put(HertzConstants.KEY, StringUtils.trim(subResourceMap
                        .get(HertzConstants.ERROR_CODE, String.class)));
		...
		errorObject.accumulate(str, errorMsgJsonArray);
