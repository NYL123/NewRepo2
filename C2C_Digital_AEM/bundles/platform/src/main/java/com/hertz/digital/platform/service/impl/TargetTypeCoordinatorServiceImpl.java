...
/**
 * This class is used to form the request for the target type notification
 * service.
 * 
 * @author himanshu.i.sharma
 *
 */
@Component(immediate = true, metatype = false, label = "Hertz - Target Type MC Data Coordinator Service", description = "Hertz - Target Type MC Data Coordinator Service")
@Service(value = MCDataCoordinatorService.class)
@Properties(value = @Property(name = "identifier", value = "targettype", propertyPrivate = true))
public class TargetTypeCoordinatorServiceImpl extends AbstractMCDataCoordinator {

	/**
	 * LOGGER instantiation
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(TargetTypeCoordinatorServiceImpl.class);
	 ...
	 */
	public TargetTypeCoordinatorServiceImpl() {
		super();
	}
	...
	private void getJsonBodyParams(JSONObject finalJsonObject, JSONObject jsonObject, Resource contentResource)
		...
		if (HertzUtils.isValidResource(parResource)) {
			...
			JSONObject targetTypeObject = new JSONObject();
			JSONArray targetTypeJsonArray = new JSONArray();
			String str = StringUtils.EMPTY;
			for (Resource subResource : resourceIterator) {
				JSONObject object = new JSONObject();
				String componentResourceType = subResource.getResourceType();
				str = contentResource.getValueMap().get("service",
						componentResourceType.substring(componentResourceType.lastIndexOf('/')).replaceAll("/", ""));
				...
				if (subResourceMap.containsKey(HertzConstants.KEY)) {
					...
					object.put(HertzConstants.VALUE,
							StringUtils.defaultIfBlank(
									...
									StringUtils.EMPTY));
					targetTypeJsonArray.put(object);
				}
			}
			targetTypeObject.accumulate(str, targetTypeJsonArray);
			JSONArray jsonArray = new JSONArray();
			jsonArray.put(targetTypeObject);
