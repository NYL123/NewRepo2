...
/**
 * This class is used to form the request for the global config notification
 * service.
 * 
 * @author himanshu.i.sharma
 *
 */
@Component(immediate = true, metatype = false, label = "Hertz - Global Config MC Data Coordinator Service", description = "Hertz - Global Config MC Data Coordinator Service")
@Service(value = MCDataCoordinatorService.class)
@Properties(value = @Property(name = "identifier", value = "globalconfig", propertyPrivate = true))
public class GlobalConfigCoordinatorServiceImpl extends AbstractMCDataCoordinator {

	/**
	 * LOGGER instantiation
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(GlobalConfigCoordinatorServiceImpl.class);
	 ...
	 */
	public GlobalConfigCoordinatorServiceImpl() {
		super();
	}
	...
	private void getJsonBodyParams(JSONObject finalJsonObject, JSONObject jsonObject, Resource contentResource)
		...
		jsonObject.put(HertzConstants.BRAND,
				StringUtils.defaultIfBlank(map.get(HertzConstants.BRAND, String.class), StringUtils.EMPTY));
		LOGGER.debug("brand :- " + jsonObject.optString(HertzConstants.BRAND));
		...
		if (HertzUtils.isValidResource(parResource)) {
			...
			JSONObject globalConfigObject = new JSONObject();
			JSONArray globalConfigJsonArray = new JSONArray();
			for (Resource subResource : resourceIterator) {
				...
				if (subResourceMap.containsKey(HertzConstants.KEY)
						&& subResourceMap.containsKey(HertzConstants.VALUE)) {
					...
					object.put(HertzConstants.VALUE,
							StringUtils.trim(subResourceMap.get(HertzConstants.VALUE, String.class)));
					globalConfigJsonArray.put(object);
				}
			}
			globalConfigObject.accumulate(HertzConstants.ENV_CONFIG, globalConfigJsonArray);
			JSONArray jsonArray = new JSONArray();
			jsonArray.put(globalConfigObject);
