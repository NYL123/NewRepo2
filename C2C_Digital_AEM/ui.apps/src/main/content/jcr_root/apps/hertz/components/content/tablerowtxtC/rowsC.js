	   ...
	   var counter = 0;
        ...
        if(null !=row.properties ){
            ...
            while(counter < columncount){
				...
				var classText2 = (null !=row && undefined != row.properties[text2Prop]) ? "table-pattern-c-sub-text" :"";
                col[counter] = selection;
                sub[counter] = subtext;
                td[counter] = "<span class='mobilelabel'>"+ columnHeaders[counter] +"</span><span class='"+ classText1 +"'>"+ selection + " </span> <span class='" + classText2 +"'>" + subtext + "</span>";

				//td[counter] = selection + subtext ;
                var header = columnHeaders[counter] ; 

                //console.log("+++++++++++++++++++++" + header + " : " + propertyName + "  : " + subPropertyName );
                mobcol[counter] = "<label class='table-pattern-c-colum-title'>" + header + ":</label> <label>" +  " " + selection + " " + subtext + "</label>"; 
				//console.log("+++++++++++++++++++++mobcol[counter] " + mobcol[counter] );
                counter= counter+1;
